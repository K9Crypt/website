<script lang="ts">
    ...
    function copyToClipboard() {
        ...
        () => toast.success('Decrypted message copied to clipboard.', { duration: 3000, position: "bottom-right", style: 'background-color: #1B1B1B; color: #fff;' }),
        err => {
            console.error('Could not copy text: ', err);
            toast.error('Failed to copy decrypted message. Please try again.', { duration: 3000, position: "bottom-right", style: 'background-color: #1B1B1B; color: #fff;' });
        }
        );
    }
    ...
    function downloadResult() {
        ...
        toast.success('File downloaded successfully.', { duration: 3000, position: "bottom-right", style: 'background-color: #1B1B1B; color: #fff;' });
    }
</script>
...
<section class="min-h-screen flex items-center justify-center py-10 md:py-0">
    <div class="container mx-auto px-4 sm:px-6 lg:px-10 max-w-2xl">
        <div class="mb-8">
            <h2 class="text-3xl font-bold mb-3">Decrypt Message</h2>
